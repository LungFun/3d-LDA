...
def VisualizeTopics(phi, words, num_topics, viz_threshold=9e-3):
	#myfont = matplotlib.font_manager.FontProperties(fname="/usr/share/fonts/X11/Type1/n019003l.pfb")
	#true_id2name = {}
	#with open('../tv_data/pid2name_tot.txt') as fp:
	#	for line in fp:
	#		id_, name = line.strip().split('\t')
	#		true_id2name[id_] = name

	phi_viz = np.transpose(phi)
	words_to_display = ~np.all(phi_viz <= viz_threshold, axis=1)
	#words_viz = [true_id2name[words[i]] for i in range(len(words_to_display)) if words_to_display[i]]
	...
	#ax.set_yticklabels(column_labels, minor=False, fontproperties=myfont)
	ax.set_yticklabels(column_labels, minor=False)
        #plt.xlabel('主题号', size =15, )
        #plt.ylabel('节目', size = 15)
	plt.savefig('topics_tv.png')
...
def VisualizeEvolution(psi):
	...
	plt.savefig('evolution_tv.png')

def main():
	resultspath = '../results_tv/pnas_tot/'
	tot_pickle_path = resultspath + 'pnas_tot.pickle'

	tot_pickle = open(tot_pickle_path, 'rb')
	par = pickle.load(tot_pickle)
	VisualizeTopics(par['n'], par['word_token'], par['T'], 0.03)
