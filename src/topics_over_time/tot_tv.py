...
class TopicsOverTime:
	def GetPnasCorpusAndDictionary(self, documents_path, timestamps_path, stopwords_path):
		...
		for timestamp in fileinput.input(timestamps_path):
			...
			timestamps.append([float(t_str) for t_str in timestamp.strip().split()])
		for line in fileinput.input(stopwords_path):
			stopwords.update(Set(line.lower().strip().split()))
		...
		first_timestamp = min([min(ts) for ts in timestamps])
		last_timestamp = max([max(ts) for ts in timestamps])
		timestamps = [[1.0*(t-first_timestamp)/(last_timestamp-first_timestamp) for t in ts] for ts in timestamps]
	...
	def GetMethodOfMomentsEstimatesForPsi(self, par):
		...
		for i in range(len(topic_timestamps)):
			...
			#	psi[i][1] = 1 + (1-timestamp_mean)*common_factor
			#else:
			#	psi[i][0] =  timestamp_mean*common_factor
			#	psi[i][1] =  (1-timestamp_mean)*common_factor
		return psi
			...
			for d in range(par['D']):
				for i in range(par['N'][d]):
					...
					try:
						...
						print(topic_probabilities, list(np.random.multinomial(1, topic_probabilities, size=1)[0]), par['psi'][topic_di])
						sys.exit(1)
