...
void model::set_default_values() {
    ...
    wordmapfile_t = "wordmap_t.txt"; //fang
    wordmapfile_g = "wordmap_g.txt";//fang
    trainlogfile = "trainlog.txt";
    tassign_suffix = ".tassign";
    theta_suffix = ".theta";
    phi_suffix = ".phi";
    phi_t_suffix = ".phi.t";//fang
    phi_g_suffix = ".phi.g";//fang
    others_suffix = ".others";
    twords_suffix = ".twords";
    twords_suffix_t = ".twords.t";//fang
    twords_suffix_g = ".twords.g";//fang
    
    dir = "./";
    dfile = "trndocs.dat";
    dfile_t ="trndocs.dat.t";//fang
    dfile_g ="trndocs.dat.g";//fang
...
int model::save_model(string model_name) {
    ...
    if (save_model_phi_t(dir + model_name + phi_t_suffix)) {//fang
	return 1;
    }
    if (save_model_phi_g(dir + model_name + phi_g_suffix)) {//fang
	return 1;
    }
	...
	if (save_model_ttime(dir + model_name + twords_suffix_t)) {//fang
	    return 1;
	}
	if (save_model_tgeo(dir + model_name + twords_suffix_g)) {//fang
	    return 1;
	}
    }
...
int model::save_model_phi_t(string filename) {
    FILE * fout = fopen(filename.c_str(), "w");
    if (!fout) {
	printf("Cannot open file %s to save!\n", filename.c_str());
	return 1;
...
int model::save_model_phi_g(string filename) {
    FILE * fout = fopen(filename.c_str(), "w");
    if (!fout) {
	printf("Cannot open file %s to save!\n", filename.c_str());
	return 1;
...
int model::save_model_ttime(string filename) {//fang
    FILE * fout = fopen(filename.c_str(), "w");
    if (!fout) {
	printf("Cannot open file %s to save!\n", filename.c_str());
	return 1;
    ...
    printf("Number of most likely timespans per topic: %i\n", twords);
    if (twords > Vt) {
	printf("Set number of most likely timespans to V: %i\n", Vt);
	twords = Vt;
    }
    mapid2word::iterator it;

    if (!id2time.size()) {
	printf("Something went wrong with loading the time, size of id2time: %zu\n", id2time.size());
	return -1;
    }

    for (int k = 0; k < L; k++) {
	vector<pair<int, double> > time_probs;
	pair<int, double> time_prob;
	for (int w = 0; w < Vt; w++) {
	    time_prob.first = w;
	    time_prob.second = phi_t[k][w];
	    time_probs.push_back(time_prob);
	}
    
        // quick sort to sort word-topic probability
	utils::quicksort(time_probs, 0, time_probs.size() - 1);
	
	fprintf(fout, "Topic %dth:\n", k);
	for (int i = 0; i < twords; i++) {
	    it = id2time.find(time_probs[i].first);
	    if (it != id2time.end()) {
		fprintf(fout, "\t%s   %f\n", (it->second).c_str(), time_probs[i].second);
	    }
	}
    }
...
int model::save_model_tgeo(string filename) {//fang
    FILE * fout = fopen(filename.c_str(), "w");
    if (!fout) {
	printf("Cannot open file %s to save!\n", filename.c_str());
	return 1;
    ...
    if (!id2geo.size()) {
	printf("Something went wrong with loading the geo, size of id2geo: %zu\n", id2geo.size());
	return -1;
    }

    for (int k = 0; k < G; k++) {
	vector<pair<int, double> > geo_probs;
	pair<int, double> geo_prob;
	for (int w = 0; w < Vg; w++) {
	    geo_prob.first = w;
	    geo_prob.second = phi_g[k][w];
	    geo_probs.push_back(geo_prob);
	}
    
        // quick sort to sort word-topic probability
	utils::quicksort(geo_probs, 0, geo_probs.size() - 1);
	
	fprintf(fout, "Topic %dth:\n", k);
	for (int i = 0; i < twords; i++) {
	    it = id2geo.find(geo_probs[i].first);
	    if (it != id2geo.end()) {
		fprintf(fout, "\t%s   %f\n", (it->second).c_str(), geo_probs[i].second);
	    }
	}
    }
...
int model::init_est() {
    ...
    ptrndata_t = new dataset; //fang
    if (ptrndata_t->read_trndata(dir + dfile_t, dir + wordmapfile_t)) {
        printf("Fail to read training data t!\n");
        return 1;
    }
    ptrndata_g = new dataset; //fang
    if (ptrndata_g->read_trndata(dir + dfile_g, dir + wordmapfile_g)) {
        printf("Fail to read training data g!\n");
        return 1;
    }
    ...
    nd = new int*[M];
    for (m = 0; m < M; m++) {
        nd[m] = new int[K * L * G]; //fang
        for (k = 0; k < K * L * G ; k++) { //fang
    	    nd[m][k] = 0;
        }
    }
    ...
    ntsum = new int[L];//fang
    for (k = 0; k < L; k++) {
	ntsum[k] = 0;
    }
    ngsum = new int[G];//fang
    for (k = 0; k < G; k++) {
	ngsum[k] = 0;
    }
	...
	z[m] = new int[N];
        ...
        // initialize for z
        for (n = 0; n < N; n++) {
    	    int topic = (int)(((double)random() / RAND_MAX) * K * L * G);//fang
    	    z[m][n] = topic;
    	    ...
    	    nw[ptrndata->docs[m]->words[n]][kk] += 1;
            nt[ptrndata_t->docs[m]->words[n]][ll] += 1; //fang
            ng[ptrndata_g->docs[m]->words[n]][gg] += 1; //fang
    ...
    phi_t = new double*[L];//fang
    for (k = 0; k < L; k++) {
        phi_t[k] = new double[Vt];
    }    
    phi_g = new double*[G];//fang
    for (k = 0; k < G; k++) {
        phi_g[k] = new double[Vg];
    }    
...
void model::estimate() {
	...
	dataset::read_wordmap(dir + wordmapfile_t, &id2time);//fang
	dataset::read_wordmap(dir + wordmapfile_g, &id2geo);//fang
    }
	...
	if (savestep > 0) {
		...
		compute_phi();
                printf("computing phi_t\n");
		compute_phi_t();//fang
                printf("computing phi_g\n");
		compute_phi_g();//fang
                printf("Saving model phase!\n");
...
int model::sampling(int m, int n) {
    ...
    int topic = z[m][n];
    int w = ptrndata->docs[m]->words[n];
    int t = ptrndata_t->docs[m]->words[n]; //fang
    int g = ptrndata_g->docs[m]->words[n]; //fang
    ...
    double Vbeta = V * beta;
    double Vbeta_t = Vt * beta_t;//fang
    double Vbeta_g = Vg * beta_g;//fang
    double Kalpha = K * L * G * alpha;//fang    
    // do multinomial sampling via cumulative method
    for (int k = 0; k < K * L * G; k++) {//fang
        kk = (int)(k / Klen);
        ll = (int)((k - kk * Klen) / Llen);
        gg = (int)(k - kk * Klen - ll * Llen);
	p[k] =     ((nw[w][kk] + beta) /   (nwsum[kk] + Vbeta)) *
                   ((nt[t][ll] + beta_t) / (ntsum[ll] + Vbeta_t)) *
                   ((ng[g][gg] + beta_g) / (ngsum[gg] + Vbeta_g)) *
		   (nd[m][k] + alpha) * 10000 ;/// (ndsum[m] + Kalpha);
    }
    // cumulate multinomial parameters, sum will be in p[K-1]
    for (int k = 1; k < K * L * G; k++) {//fang
	p[k] += p[k - 1];
        //printf("p[%d]:%f,",k,p[k]);
    }
    // scaled sample because of unnormalized p[]
    double u = ((double)random() / RAND_MAX) * p[K * L * G - 1];
    //printf("\n u is:%f\n", u);
    
    // get topic with probability beyond u 
    for (topic = 0; topic < K * L * G; topic++) {//fang
	if (p[topic] > u) {
	    break;
	}
    }
    ...
    kk = (int)(topic / Klen);
    ll = (int)((topic - kk * Klen) / Llen);
    gg = (int)(topic - kk * Klen - ll * Llen);
    // add newly estimated z_i to count variables
    //printf("adding topic %d:(%d,%d,%d)\n", topic, kk,ll,gg);
...
void model::compute_theta() {
    for (int m = 0; m < M; m++) {
	for (int k = 0; k < K * L * G; k++) { // fang
           //printf("in computing theta %d/%d: %d + %f / %d + %f\n", m,k, nd[m][k], alpha, ndsum[m], K * alpha);
	    theta[m][k] = (nd[m][k] + alpha) / (ndsum[m] + K * L * G * alpha);
	}
    }
}
...
void model::compute_phi_t() {//fang
    //printf("k:%d, Vt:%d", k, Vt);
    for (int k = 0; k < L; k++) {
	for (int w = 0; w < Vt; w++) {
            //fflush(stdout);
            //printf("in computing phi_t %d/%d: %d + %f / %d + %f\n", k,w, nt[w][k], beta_t, ntsum[k], Vt * beta_t);
	    phi_t[k][w] = (nt[w][k] + beta_t) / (ntsum[k] + Vt * beta_t);
	}
    }
}

void model::compute_phi_g() {//fang
    for (int k = 0; k < G; k++) {
	for (int w = 0; w < Vg; w++) {
           //printf("in computing phi_g %d/%d: %d + %f / %d + %f\n", k,w, ng[w][k], beta_g, ngsum[k], Vg * beta_g);
	    phi_g[k][w] = (ng[w][k] + beta_g) / (ngsum[k] + Vg * beta_g);
	}
    }
}
int model::init_inf() {
