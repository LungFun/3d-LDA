...
public:
    ...
    string wordmapfile_t;  //**
    string wordmapfile_g;  //**
    ...
    string twords_suffix_t;// **
    string twords_suffix_g;// **
    ...
    int Vt;// timespan vocabulary size    **
    int Vg;// geo vocabulary size         **
    int K; // number of topics 
    int L;//number of timespans *
    int G;//number of geos      *
    double alpha, beta; // LDA hyperparameters 
    double beta_t, beta_g;// add for t and g
    ...
    int ** nt;// nt[i][j]: number of instances of timespan i assigned to topic j,size L x K  **
    int ** ng;// ng[i][j]: number of instances of geo i assigned to topic j, size G * K      **
    ...
    int * ntsum;// ntsum[j]: total number of timespans assigned to topic j, size K           **
    int * ngsum;// ngsum[j]: total number of geo assigned to topic j, size K                 **
    ...
    double ** phi_t; //pi: topic-timespan distributions, size K x L                             **
    double ** phi_g;// ga: topic-geo distributions, size     K x G                              **
    ...
    int save_model_phi_t(string filename);//fang
    int save_model_phi_g(string filename);//fang
    int save_model_others(string filename);
    int save_model_twords(string filename);
    int save_model_ttime(string filename);//fang
    int save_model_tgeo(string filename);//fang
    ...
    void compute_theta();
    void compute_phi();
    void compute_phi_t();//fang
    void compute_phi_g();//fang
