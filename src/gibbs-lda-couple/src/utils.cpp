...
int utils::parse_args(int argc, char ** argv, model * pmodel) {
    ...
    double alpha = -1.0;
    double beta = -1.0;
    double beta_t = -1.0;//fang
    double beta_g = -1.0;//fang
    int K = 0;
    int L = 0;//fang
    int G = 0;//fang
    int niters = 0;
	...
	if (arg == "-est") {
	...
	} else if (arg == "-dfile_t") {//fang
	    dfile_t = argv[++i];	    
	} else if (arg == "-dfile_g") {//fang
	    dfile_g = argv[++i];	    
	} else if (arg == "-model") {
	    model_name = argv[++i];	    	    
	    ...
	    beta = atof(argv[++i]);	    
	} else if (arg == "-beta_t") {//fang
	    beta_t = atof(argv[++i]);	    
	} else if (arg == "-beta_g") {//fang
	    beta_g = atof(argv[++i]);	    
	    
	} else if (arg == "-ntopics") {
	    K = atoi(argv[++i]);	    
	} else if (arg == "-ntimes") {//fang
	    L = atoi(argv[++i]);	    
	} else if (arg == "-ngeos") {//fang
	    G = atoi(argv[++i]);	    
	...
	if (dfile_t == "") {
	    printf("Please specify the input data_t file for model estimation!\n");
	    return 1;
	}
	if (dfile_g == "") {
	    printf("Please specify the input data_g file for model estimation!\n");
	    return 1;
	}
	...
	if (alpha >= 0.0) {
	    ...
	    pmodel->alpha = 50.0 / (pmodel->K * pmodel->L * pmodel->G);
	}
	...
	if (beta_t >= 0.0) {//fang
	    pmodel->beta_t = beta_t;
	}
	if (beta_g >= 0.0) {//fang
	    pmodel->beta_g = beta_g;
	}
	...
	pmodel->dfile_t = dfile_t;//fang

	pmodel->dfile_g = dfile_g;//fang
